<div class="panel-heading">
    ...
    <div class="heading-elements">    
        ...
        <div class="heading-btn">
            <a href="{!!route(Helper::getNameOfPage('create3'),['uuid'=>$rkpd->RKPDID])!!}" class="btn btn-success btn-xs" title="Tambah Rincian Kegiatan dari POKIR">
                <i class="icon-googleplus5"></i>
            </a>
            <a href="{!!route(Helper::getNameOfPage('create4'),['uuid'=>$rkpd->RKPDID])!!}" class="btn btn-primary btn-xs" title="Tambah Rincian Kegiatan">
                <i class="icon-googleplus5"></i>
            </a>
        </div>  
        @endif        
    ...
    <table id="data" class="table table-striped table-hover" style="font-size:11px">
        <thead>
            <tr class="bg-teal-700">
                ...
                <th>TARGET (%)</th> 
                <th class="text-right">NILAI USULAN<br>RKPD MURNI</th>                
                <th class="text-right">NILAI USULAN<br>PEMBAHASAN RKPD</th>                
                ...
                <td>                   
                    <ul class="icons-list"> 
                        <li class="text-primary-600">
                            <a class="btnShow" href="{{route(Helper::getNameOfPage('showrincian'),['uuid'=>$item->RKPDRincID])}}" title="Detail Data Usulan Musren Kabupaten">
                                <i class='icon-eye'></i>
                            </a>  
                        </li>
                        @if ($item->Privilege==0)                   
                        <li class="text-primary-600">
                            @if ($item->isSKPD)
                                <a class="btnEdit" href="{{route(Helper::getNameOfPage('edit4'),['uuid'=>$item->RKPDRincID])}}" title="Ubah Data Usulan {{$page_title}}">
                                    <i class='icon-pencil7'></i>
                                </a> 
                            @elseif($item->isReses)
                                <a class="btnEdit" href="{{route(Helper::getNameOfPage('edit3'),['uuid'=>$item->RKPDRincID])}}" title="Ubah Data Usulan {{$page_title}}">
                                    <i class='icon-pencil7'></i>
                                </a>
                            @elseif(!empty($item->UsulanKecID))
                                <a class="btnEdit" href="{{route(Helper::getNameOfPage('edit2'),['uuid'=>$item->RKPDRincID])}}" title="Ubah Data Usulan {{$page_title}}">
                                    <i class='icon-pencil7'></i>
                                </a>
                            @else
                                <a class="btnEdit" href="{{route(Helper::getNameOfPage('edit4'),['uuid'=>$item->RKPDRincID])}}" title="Ubah Data Usulan {{$page_title}}">
