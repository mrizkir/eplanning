        ...
        <table id="data" class="table table-striped table-hover">
            ...
            @foreach ($data as $key=>$item)
                <tr>
                    ...
                    <td>{{DB::table('v_rkpd')->where('OrgID',$item->OrgID)->where('EntryLvl',100)->count()}}</td>
                    <td>{{DB::table('v_rkpd')->where('OrgID',$item->OrgID)->where('EntryLvl',100)->count()}}</td>
                    <td>{{Helper::formatUang(DB::table('v_rkpd')->where('OrgID',$item->OrgID)->where('EntryLvl',100)->sum('NilaiUsulan3'))}}</td>
                    <td>{{Helper::formatUang(DB::table('v_rkpd')->where('OrgID',$item->OrgID)->where('EntryLvl',100)->sum('NilaiUsulan4'))}}</td>
                    <td>{{$item->TA}}</td>
                    <td>
                        <ul class="icons-list">
