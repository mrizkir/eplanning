...
{   
    public function __construct($dataReport,$print=true)
    {
        ...
        $this->spreadsheet->getProperties()->setTitle('Laporan '.$dataReport['sheetname'].' Tahun '.\HelperKegiatan::getTahunPerencanaan());
        $this->spreadsheet->getProperties()->setSubject('Laporan '.$dataReport['sheetname'].' Tahun '.\HelperKegiatan::getTahunPerencanaan()); 
        if ($print)
        {
            $this->print();             
        }        
    ...
    private function  print()  
    {
        ...
        $sheet->setTitle ("LAPORAN $sheetname TA ".\HelperKegiatan::getTahunPerencanaan());   
        ...
        foreach ($struktur as $Kd_Urusan=>$v1)
        {
            ...
            if ($Kd_Urusan == 0)
            {
                ...
                {   
                    $daftar_kegiatan = \DB::table('trRenja')
                                                ->select(\DB::raw('"trRenja"."KgtID","tmKgt"."Kd_Keg","tmKgt"."KgtNm"'))
                    ...
                    if (count($daftar_kegiatan)  > 0)
                    {
                        ...
                        foreach ($daftar_kegiatan as $v4) 
                        {
                            ...
                            $sheet->setCellValue("N$row",\Helper::formatUang($renja->NilaiSetelah)); 
                            $total_nilai_setelah+=$renja->NilaiSetelah;  
                            $totaleachprogram_setelah+=$renja->NilaiSetelah;
                            ...
                            foreach ($rincian_kegiatan as $v5)
                            {
                                ...
                                $sheet->setCellValue("H$row",'Kab. Bintan'); 
                                $sasaran_angka=\Helper::formatAngka($v5->Sasaran_Angka);
                                $sheet->setCellValue("I$row",trim(preg_replace('/[\t\n\r\s]+/', ' ', $sasaran_angka.' '.$v5->Sasaran_Uraian)));                                                                        
                                $sheet->setCellValue("J$row",\Helper::formatUang($v5->Jumlah)); 
                                $sheet->setCellValue("K$row",$v5->Nm_SumberDana); 
                                $sheet->setCellValue("L$row",$v5->Descr); 
                                $sheet->setCellValue("M$row",\Helper::formatAngka($renja->Sasaran_AngkaSetelah).' '.$renja->Sasaran_UraianSetelah); 
                                $sheet->setCellValue("N$row",\Helper::formatUang($renja->NilaiSetelah)); 
                                $total_pagu+=$v5->Jumlah;
                                $totaleachkegiatan+=$v5->Jumlah;
                                $no+=1;
                                $row+=1;
                            }
