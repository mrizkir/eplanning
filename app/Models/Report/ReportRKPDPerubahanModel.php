...
{   
    ...
    private function printPembahasanRKPDP()  
    {
        ...
        {
            $PrgID=$v->PrgID;         
            $daftar_kegiatan = RKPDViewJudulModel::select(\DB::raw('"kode_kegiatan","KgtNm","Sasaran_Angka1","Sasaran_Uraian3","Target1","Target3","NilaiUsulan1","NilaiUsulan3","Sasaran_AngkaSetelah","Sasaran_UraianSetelah","NilaiSetelah","Nm_SumberDana","Descr"'))
                                            ->where('PrgID',$PrgID)      
                                            ->where('OrgID',$OrgID);
            $daftar_kegiatan = ($SOrgID == 'none' || $SOrgID == '') ?
                                                                    $daftar_kegiatan->where('TA',\HelperKegiatan::getTahunPerencanaan())
            ...
            {   
                ...
                foreach ($daftar_kegiatan as $n) 
                {
                    ...
                    $sheet->setCellValue("O$row",\Helper::formatUang($n['NilaiUsulan3']-$n['NilaiUsulan1'])); 
                    $sheet->setCellValue("P$row",$n['Descr']); 
